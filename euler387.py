...
# miller-rabin primality test (Miller, Rabin, Miller-Rabin, def miller)
  ...
  for x in todo:
    ...
    for digit in xrange(10):
      ...
      if is_strong_parent and is_prime(current):
        result += current
      dig_sum = digit_sum(current)
      if current % dig_sum == 0:
        next_todo.append(current)
        if is_prime(current // dig_sum):
          strong.add(current)
  todo = next_todo
print result
